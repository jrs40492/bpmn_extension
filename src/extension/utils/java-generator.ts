...
function generateFieldExtractionCode(
  ...
  const fieldSetters = event.fields.map(field => {
    ...
    return `${indent}    setVariable(pi, "${fieldName}", typedPayload.${getterName}());
...
${indent}        setVariable(pi, "${fieldName}", ${fieldName}Value);
 ...
 *
 * IMPORTANT: Uses afterNodeLeft for ALL message events (including start events).
 * This is critical because:
 * - afterNodeTriggered fires BEFORE dataOutputAssociations are processed
 * - afterProcessStarted fires BEFORE the start event node completes
 * - afterNodeLeft fires AFTER the node completes and dataOutputAssociations are processed
 *
 * Start events ARE nodes in the process graph, so afterNodeLeft fires for them too.
 *
...
export function generatePayloadExtractorListener(
  ...
  // Generate extraction code for ALL events (used in afterNodeLeft)
  // Start events are nodes too, and afterNodeLeft fires after they complete
